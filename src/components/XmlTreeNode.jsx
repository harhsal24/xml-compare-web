...
export default function XmlTreeNode({ node, side, depth = 0 }) {
    ...
    const { diffResults, selectedXPath, setSelectedXPath } = useXmlStore();
    ...
    // Color classes based on diff status
    const statusColors = {
    ...
    const attrsDisplay = Object.entries(node.attributes)
    ...
    return (
        ...
        `}
            ...
            >
                ...
                {/* Attributes */}
                {attrsDisplay && (
                    <span className="text-purple-600"> {attrsDisplay}</span>
                )}
                ...
                {node.textContent && !hasChildren && (
                    <span className="text-gray-700 ml-1 truncate max-w-xs">{node.textContent}</span>
                )}
            ...
            {hasChildren && expanded && (
                <div className="ml-2 border-l-2 border-slate-200/50">
                    ...
                    {/* Closing tag */}
                    <div
                        className="text-blue-600 font-mono text-sm py-0.5 px-2 opacity-50"
